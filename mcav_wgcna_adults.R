...
datTraits$C1 = ifelse(datTraits$admix== "N1",1,0)
datTraits$C2 = ifelse(datTraits$admix== "O2",1,0)
datTraits$C3 = ifelse(datTraits$admix== "D3",1,0)

datTraits$N1N = ifelse(datTraits$admix.habitat=="N1nearshore",1,0)
datTraits$N1O = ifelse(datTraits$admix.habitat=="N1offshore",1,0)
datTraits$O2O = ifelse(datTraits$admix.habitat=="O2offshore",1,0)
datTraits$O2N = ifelse(datTraits$admix.habitat=="O2nearshore",1,0)
datTraits$D3D = ifelse(datTraits$admix.habitat=="D3deep",1,0)
datTraits$D3O = ifelse(datTraits$admix.habitat=="D3offshore",1,0)

datTraits = datTraits %>%
  select(D,N,O,C1,C2,C3,N1N,N1O,O2O,O2N,D3D,D3O)
save(datTraits, datExpr, file = "mcav_files/wgcna_adults_input.Rdata")
...
save(datt,traits,file="mcav_files/wgcnaData.adult.RData")
...
dissTOM = 1-TOM
save(TOM,dissTOM,file="mcav_files/TOM.adult.rdata")
...
#merge modules
MEDissThres = 0.8 # in the first pass, set this to 0 - no merging (we want to see the module-traits heatmap first, then decide which modules are telling us the same story and better be merged)
sizeGrWindow(7, 6)
...
save(MEs, geneTree, moduleLabels, moduleColors, file = "mcav_files/networkdata_signed.adult.RData")
...
Traits=datTraits[,c("C1", "C2", "C3","N", "O", "D")]
traitlabels <- c("Nearshore\nEcomorph", "Offshore\nEcomorph", "Deep 1\nEcomorph","Nearshore\nHabitat", "Offshore\nHabitat", "Deep\nHabitat")
...
Traits2=datTraits[,c("N1N", "O2N","N1O", "O2O", "D3O", "D3D")]
traitlabels <- c("Nearshore Ecomorph\nNearshore Habitat", "Offshore Ecomorph\nNearshore Habitat","Nearshore Ecomorph\nOffshore Habitat", 
                 "Offshore Ecomorph\nOffshore Habitat", "Deep 1 Ecomorph\nOffshore Habitat", "Deep 1 Ecomorph\nDeep Habitat")
...
par(mar=c(4, 4.2, 4, 1))
barplot(ME, col=which.module, main="Midnightblue Module", cex.main=1,
...
datTraits3 <- datTraits[order(datTraits$C3,datTraits$C2,datTraits$C1),]
...
midnightblue_df <- merge(MEs, coldata, by = "row.names")
midnightblue_df <- na.omit(midnightblue_df)
##order by lineage
midnightblue_df$admix <- factor(midnightblue_df$admix, levels = c("N1","O2","D3"))
midnightblue_df <- na.omit(midnightblue_df)
ggplot(midnightblue_df, aes(x = admix, y = MEmidnightblue, color = admix))+
  geom_boxplot(width = 0.5, outlier.shape = NA)+
  theme_bw()+
  theme(panel.grid = element_blank(),  axis.text.x = element_blank())+
  ggforce::geom_sina(maxwidth = 0.3)+
  scale_color_manual(name = "Ecomorph", labels = c("Nearshore","Offshore", "Deep 1"), values = c('palegreen1','plum1','skyblue')) +
  scale_y_continuous(name=expression("Midnightblue Module Eigengene"))+
  scale_x_discrete(name=expression("Ecomorph"))

#order by habitat
midnightblue_df$habitat <- factor(midnightblue_df$habitat, levels = c("nearshore","offshore","deep"))
ggplot(midnightblue_df, aes(x = habitat, y = MEmidnightblue, color = habitat))+
  geom_boxplot(width = 0.5, outlier.shape = NA)+
  theme_bw()+
  theme(panel.grid = element_blank(),  axis.text.x = element_blank())+
  ggforce::geom_sina(maxwidth = 0.3)+
...
whichModule="midnightblue" #make one for each module
...
write.csv(combo,file=paste("mcav_go/",whichModule,"_full.csv",sep=""),row.names=F,quote=F)
...
write.csv(combo,file=paste("mcav_go/",whichModule,".csv",sep=""),row.names=F,quote=F)
